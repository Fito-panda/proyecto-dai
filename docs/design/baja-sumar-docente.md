...
| 5 | `FormBuilder.gs` | `setCollectEmail(true)` para captura email logueado. **REALIDAD TÉCNICA descubierta paso 15 sesión 2 vuelta 10 (2026-04-30):** la API Apps Script Forms NO expone `setEmailCollectionType(VERIFIED)` ni equivalente — solo el booleano `setCollectEmail`. En cuenta Gmail común (NO Workspace) el comportamiento empírico observado es **VERIFIED-like con auto-captura del email logueado** + col `Email Address` en el Sheet de respuestas. NO requiere migración Workspace (que NO solucionaría nada — VERIFIED-only solo se setea via UI manual del form, lo cual rompe idempotencia FormBuilder). El nombre EXACTO del campo en `e.namedValues` es `'Email Address'` (clave para `handleAuthCheck` paso 16). Honor system con paso 16 validando `e.namedValues['Email Address']` contra `👥 Docentes` activas = arquitectura correcta del proyecto, NO parche. Plan v3 anterior decía "Modo VERIFIED + NO setRequireLogin deprecated" asumiendo API que no existe — corregido aquí. |
