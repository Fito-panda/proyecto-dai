...
| 5 | `FormBuilder.gs` | `setCollectEmail(true)` para captura email del usuario que envía. **REALIDAD TÉCNICA descubierta pasos 15-16 sesión 2 vueltas 10-12 (2026-04-30):** la API Apps Script Forms NO expone `setEmailCollectionType(VERIFIED)` ni equivalente — solo el booleano `setCollectEmail`. En cuenta Gmail común (NO Workspace) el comportamiento empírico real es **campo "Correo" obligatorio con tipeo manual del usuario**. El navegador (Chrome) puede sugerir el email logueado vía autocomplete del browser (UX del navegador, NO del form), pero NO hay auto-fill estricto del form. Diagnóstico inicial vueltas 10-11 dijo erróneamente "VERIFIED-like auto-captura del logueado" / "auto-fill propietary cuenta owner" — **corrección vuelta 12 (Fito cazó):** lo observado fue confusión entre Chrome browser autocomplete y form behavior. NO hay diferencia funcional entre owner y external más allá del autocomplete del navegador (que aparece solo cuando la cuenta logueada del browser coincide con la cuenta del usuario). **Honor system PURO** con todos los usuarios tipeando manual (o aceptando sugerencia Chrome cuando aplica). NO requiere migración Workspace (VERIFIED-only solo se setea via UI manual del form, rompe idempotencia FormBuilder). El nombre EXACTO del campo capturado en `e.namedValues` es `'Email Address'` (clave para `handleAuthCheck` paso 16). Paso 16 validando `e.namedValues['Email Address']` contra `👥 Docentes` activas = arquitectura correcta del proyecto para target Gmail común (escuelas rurales argentinas + grupo cerrado 7-10 personas conocidas). Coherente con cazada I "solo registradas operan" + cazada G "puerta no muro". Plan v3 anterior decía "Modo VERIFIED + NO setRequireLogin deprecated" asumiendo API que no existe — corregido aquí. |
